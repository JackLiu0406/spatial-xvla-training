...
import io, os, numpy as np, pyarrow.parquet as pq, av, cv2
...
# cv2.imdecode returns BGR. Legacy pipeline fed that BGR straight to
# Florence-2/DA3 (which expect RGB) — a latent R<->B swap. Set XVLA_RGB_INPUT=1
# to correctly convert BGR->RGB here (and drop the matching [::-1] flip in the
# eval encode_obs) so the whole pipeline is true RGB. Default 0 = legacy BGR,
# preserved so the old BGR-trained checkpoints still evaluate consistently.
_RGB_INPUT = os.environ.get("XVLA_RGB_INPUT", "0") == "1"
...
def decode_image_from_bytes(x) -> Image.Image:
    ...
    rgb = cv2.imdecode(x, cv2.IMREAD_COLOR)   # cv2 decodes to BGR
    if rgb is None:
        ...
        rgb = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)   # true RGB for Florence-2/DA3

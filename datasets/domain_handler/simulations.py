...
class RobotWin2Handler(BaseHDF5Handler):
    ...
    def get_camera_poses(self, f: h5py.File):
        """
        Read per-frame extrinsics + intrinsics for the cameras listed in
        meta['observation_key']. Roboreal/RobotWin2 store them at
        ``observation/<cam>/extrinsic_cv`` (T,3,4 — OpenCV world-to-camera)
        and ``observation/<cam>/intrinsic_cv`` (T,3,3 — standard K).

        Returns (extrinsics_4x4_list, intrinsics_list):
          extrinsics_4x4_list: list of [T,4,4] np.float32 arrays, padded from 3x4
          intrinsics_list:     list of [T,3,3] np.float32 arrays
        Or (None, None) if any camera lacks pose data — the caller falls back
        to the unposed path.
        """
        keys: Sequence[str] = self.meta["observation_key"]
        extrinsics, intrinsics = [], []
        for k in keys:
            # k is "observation/<cam>/rgb"; derive prefix "observation/<cam>".
            prefix = k.rsplit("/", 1)[0]
            ext_k = f"{prefix}/extrinsic_cv"
            int_k = f"{prefix}/intrinsic_cv"
            if ext_k not in f or int_k not in f:
                return None, None
            ext = f[ext_k][()].astype(np.float32)   # [T,3,4]
            T = ext.shape[0]
            ext4 = np.zeros((T, 4, 4), dtype=np.float32)
            ext4[:, :3, :] = ext
            ext4[:, 3, 3] = 1.0
            extrinsics.append(ext4)
            intrinsics.append(f[int_k][()].astype(np.float32))  # [T,3,3]
        return extrinsics, intrinsics

    def read_instruction(self, f: h5py.File) -> str:
        """Roboreal stores instructions in sibling JSON files rather than
        inside each HDF5 (so the raw data can stay read-only). For every
        ``<config>/data/episodeN.hdf5`` there is a matching
        ``<config>/instructions/episodeN.json`` containing
        ``{"seen": [...], "unseen": [...]}``. We sample one instruction
        uniformly from ``seen`` (with ``unseen`` as a fallback) so each
        epoch sees the natural language variation in the dataset.
        Falls back to the parent class' in-HDF5 lookup if the sidecar
        JSON is missing or unreadable.
        """
        try:
            hdf5_path = getattr(f, "filename", None)
            if hdf5_path:
                ep_name = os.path.basename(hdf5_path)             # episodeN.hdf5
                if ep_name.endswith(".hdf5"):
                    stem = ep_name[: -len(".hdf5")]               # episodeN
                    cfg_dir = os.path.dirname(os.path.dirname(hdf5_path))  # .../<config>
                    json_path = os.path.join(cfg_dir, "instructions", f"{stem}.json")
                    if os.path.exists(json_path):
                        with open(json_path) as jf:
                            payload = json.load(jf)
                        pool = payload.get("seen") or payload.get("unseen") or []
                        if isinstance(pool, list) and pool:
                            return random.choice(pool)
        except Exception:
            pass
        # Fallback to the in-HDF5 key (works for any handler-injected file).
        return super().read_instruction(f)

...
class EE6DActionSpace(BaseActionSpace):
    ...
    def compute_loss(self, pred, target):
        ...
        # Gripper BCE — binarize the target. X-VLA paper: the gripper state is
        # binarized. RoboReal H5 grippers are continuous [0,1]; soft-target BCE
        # floors the loss at ~0.033, so threshold at 0.5 for crisp 0/1 labels.
        g_losses = [self.bce(pred[:, :, gi], (target[:, :, gi] > 0.5).float()) for gi in self.gripper_idx]

#!/usr/bin/env bash
# DA3-BASE K=160, dual-resolution image pipeline:
#   • VLM (Florence-2): 224×224 (unchanged, required by X-VLA processor)
#   • DA3-BASE encoder: 252×336 — aspect-preserved upscale of native 240×320
...
#
# Cell purpose: fast sanity that aspect-preserved DA3 input gives a real
# improvement, using DA3-BASE (cheap, fast s/it) at K=160 (matches the K used
# in DA3-LARGE / VGGT baselines).
# Recipe: bs=32×4 H200, lr=1e-4, learning_coef=0.1, freeze=1000, warmup=2000,
#         cosine→0.1×, iters=100k, save_interval=10k.
set -eo pipefail

SRC=${REPO_ROOT}
RUN=${RUNS_ROOT}/baseline_DA3BASE_K160_native_aspect
...
grep -q '"da3_process_res": 252'                      "$BASE_CKPT/config.json" || { echo "[err] process_res not 252"; exit 1; }

TS=$(date +%Y%m%d_%H%M%S)
OUT=$RUN/runs/da3base_k160_aspect_${TS}
LOG=$RUN/_logs/da3base_k160_aspect_${TS}.log
mkdir -p "$OUT"
echo "$LOG" > /tmp/da3base_k160_aspect_log_path
echo "[$(date '+%H:%M:%S')] launching DA3-BASE K=160 aspect-preserved (VLM 224×224 + DA3-BASE 252×336) — log: $LOG"
...
nohup env CUDA_VISIBLE_DEVICES=4,5,6,7 \
  ...
  envs/xvla/bin/accelerate launch \
    ...
    > "$LOG" 2>&1 &
disown $!
echo "[$(date '+%H:%M:%S')] DA3-BASE K=160 aspect-preserved launched on GPUs 4-7"

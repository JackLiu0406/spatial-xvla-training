...
def build_optimizer(model: XVLA, lr: float, weight_decay: float, betas=(0.9, 0.95), lr_coef_soft=1.0):
    ...
    # DA3-XVLA: geometry conditioner + cross-attention fusion group (empty when
    # geometry conditioning is disabled -> baseline groups unchanged).
    geometry_params = []
    ...
    exclude = set(map(id, vlm_params + soft_prompt_params + action_params + geometry_params))
...
def main(args):
    ...
    # DA3-XVLA: resolve geometry config through a possibly PEFT-wrapped model.
    _base = getattr(model, "base_model", model)
    _base = getattr(_base, "model", _base)
    _geo_cfg = (
        getattr(_base, "config", None) and _base.config.geometry_conditioning
        if getattr(_base, "geometry_enabled", False) else None
    )
    train_dataloader = create_dataloader(
        ...
        training=True,
        geometry_conditioning=_geo_cfg,
    )
